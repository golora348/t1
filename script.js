...
async function generateCertificate() {
  ...
  if (name) {
    ...
    // 난이도 추가
    ctx.font = "24px 'NotoSerifKR-Regular'";
    ctx.fillStyle = "black";
    ...
    // 폰트 설정
    ctx.font = `${fontSize}px 'NotoSerifKR-Regular'`;
    ...
    // 가로 중앙 위치
    const nameX = canvas.width / 2; // 정 중앙 X 위치
    const nameY = canvas.height * 0.62; // 세로 위치

    ctx.textAlign = "center"; // 가운데 정렬 설정
    ctx.fillText(name, nameX, nameY); // 이름 추가
    ...
    const formattedHours = String(today.getHours() % 12 || 12).padStart(2, "0");
    ...
    // 폰트 설정
    ctx.font = "54px 'Allison-Regular'";
    ctx.textAlign = "center";
    ctx.fillText(dateString, dateX, dateY);

    // 파일 이름에 사용할 날짜와 시간 포맷
    const formattedDateTime = `${year}${
      monthIndex + 1
    }${day}${formattedHours}${minutes}${seconds}`;
    ...
    link.download = `${formattedDateTime}_${name}.png`; // 다운로드 파일 이름 설정
    link.click(); // 다운로드 실행
  }
}
            ...
            <table border="1" style="width: 100%;">
                ...
                <tbody>
                    ${Array.from(selectedQuestions)
                      .map((question, index) => {
                        const isCorrect =
                          answersChosen[index] === question.correct;
                        const resultSymbol = isCorrect ? "O" : "X";
